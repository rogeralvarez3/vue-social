      ...
      <v-window-item value="perfil">
        <v-card class="pa-5">
          ...
          <v-form @submit.prevent="guardarPerfil">
            ...
            <p class="text-caption text-medium-emphasis mb-1">Tipo de cuenta</p>
            <v-btn-toggle v-model="perfilForm.accountType" color="primary" mandatory divided class="mb-4" density="comfortable">
            ...
            <div class="d-flex ga-2">
              <v-text-field v-model="perfilForm.country" label="País" prepend-inner-icon="mdi-earth" />
              <v-text-field v-model="perfilForm.city" label="Ciudad" prepend-inner-icon="mdi-city" />
            </div>
            <p class="text-caption text-medium-emphasis mt-n2 mb-3">
              Tu ciudad determina qué publicaciones ves por defecto en el feed.
            </p>
...
const perfilForm = reactive({
  ...
  city: auth.user?.city || '',
  accountType: auth.user?.accountType || 'personal'
});
...
async function guardarPerfil() {
  ...
  try {
    ...
    formData.append('country', perfilForm.country);
    formData.append('city', perfilForm.city);
    formData.append('accountType', perfilForm.accountType);
