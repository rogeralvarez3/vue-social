...
## Feed filtrado por ciudad

Al registrarse, cada usuario debe indicar **país y ciudad** (`POST
/api/auth/register` ahora exige `country` y `city`). El feed de
publicaciones (`GET/POST /api/posts` con `action: 'listar'`) filtra por
defecto usando un parámetro `alcance`:

- `alcance=mi_ciudad` (**por defecto**): solo publicaciones de usuarios de la
  misma ciudad que el usuario autenticado.
- `alcance=todas`: publicaciones de cualquier ciudad.
- `alcance=ciudad_especifica&ciudad=NombreDeCiudad`: publicaciones de una
  ciudad concreta (útil para "espiar" otra ciudad sin cambiar la propia).

Este filtro **solo aplica al feed general**; si se pide `userId` (ver el
perfil de alguien en particular) se muestran todas sus publicaciones sin
importar la ciudad. Para poblar el selector de "otra ciudad" en el frontend,
`POST/GET /api/users` con `action: 'listar'` y `listarCiudades: true`
devuelve la lista de ciudades que existen entre los usuarios registrados.
