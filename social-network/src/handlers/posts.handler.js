...
async function listar(req, res) {
  ...
  const { userId, page = 1, limit = 10, alcance = 'mi_ciudad', ciudad } = source;
  const offset = (Number(page) - 1) * Number(limit);
  const viewerId = req.userId || null;

  const params = [viewerId, Number(limit), offset];
  let where = 'WHERE p.deleted_at IS NULL';
  let cityFilterAplicado = null;

  if (userId) {
    // Viendo el perfil de una persona especifica: se muestran todas sus
    // publicaciones, sin aplicar el filtro de ciudad (ese filtro es solo
    // para el feed general segun lo pide el punto 3 de mas abajo).
    params.push(userId);
    where += ` AND p.user_id = $${params.length}`;
  } else {
    // Feed general: por defecto solo se muestran publicaciones de la misma
    // ciudad del usuario. Puede pedir "todas" o una ciudad especifica.
    if (alcance === 'ciudad_especifica' && ciudad) {
      cityFilterAplicado = ciudad;
    } else if (alcance !== 'todas' && viewerId) {
      const yo = await pool.query('SELECT city FROM users WHERE id = $1', [viewerId]);
      cityFilterAplicado = yo.rows[0]?.city || null;
    }

    if (cityFilterAplicado) {
      params.push(cityFilterAplicado);
      where += ` AND u.city = $${params.length}`;
    }
  }
  ...
  res.json({
    ...
    alcance: userId ? null : (cityFilterAplicado ? (alcance === 'ciudad_especifica' ? 'ciudad_especifica' : 'mi_ciudad') : 'todas'),
    ciudad: cityFilterAplicado
  });
}
